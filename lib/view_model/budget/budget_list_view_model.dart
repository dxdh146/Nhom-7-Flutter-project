  ...
  Future<double> calculateSpentAmount(
    ...
    if (user != null) {
      ...
      switch (budget.repeat) {
        case Repeat.Daily:
          cycleEndDate = cycleStartDate.add(const Duration(days: 1)).subtract(const Duration(seconds: 1));
          break;
        case Repeat.Weekly:
          cycleEndDate = cycleStartDate.add(const Duration(days: 7)).subtract(const Duration(seconds: 1));
          break;
        case Repeat.Monthly:
      ...
      while (cycleStartDate.isBefore(currentDate) && cycleStartDate.isBefore(endDate)) {
        ...
        for (var transaction in transactions) {
          if (transaction.date.isAfter(cycleStartDate.subtract(const Duration(days: 1))) &&
              transaction.date.isBefore(cycleEndDate.add(const Duration(seconds: 1))) &&
        ...
        switch (budget.repeat) {
          case Repeat.Daily:
            cycleStartDate = cycleEndDate.add(const Duration(seconds: 1));
            cycleEndDate = cycleStartDate.add(const Duration(days: 1)).subtract(const Duration(seconds: 1));
            break;
          case Repeat.Weekly:
            cycleStartDate = cycleEndDate.add(const Duration(seconds: 1));
  ...
  String getDisplayTime(Budget budget) {
    ...
    switch (budget.repeat) {
      ...
      // Tính tuần bắt đầu hiện tại dựa trên ngày bắt đầu hạn mức
        ...
        if (currentDate.isBefore(startDate)) {
          final initialWeekEndDate = startDate.add(const Duration(days: 6));
    ...
    switch (budget.repeat) {
      case Repeat.Daily:
        ...
        while (dailyStartDate.isBefore(currentDate) &&
          ...
          dailyStartDate = dailyStartDate.add(const Duration(days: 1));
        }
