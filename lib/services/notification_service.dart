...
class NotificationService {
  ...
  //   tz.setLocalLocation(
  //       tz.getLocation('Asia/Ho_Chi_Minh')); // Thiết lập múi giờ cho Việt Nam
  ...
  //     DateTime date, TimeOfDay time, Repeat repeat) async {
  //   try {
  ...
  //     DateTime date, TimeOfDay time, Repeat repeat) {
  ...
  //       (scheduledDate.day == now.day && scheduledDate.isBefore(now))) {
  //     print(
  //         'Scheduled time is before now or the same day but past current time. Adjusting time...');
  //     switch (repeat) {
  //       case Repeat.Daily:
  ...
  //             scheduledDate.month + 1, date.day, time.hour, time.minute);
  //         break;
  //       case Repeat.Quarterly:
  //         scheduledDate = _nextQuarterlyDate(scheduledDate, date.day, time);
  //         break;
  //       case Repeat.Yearly:
  //         scheduledDate = tz.TZDateTime(tz.local, scheduledDate.year + 1,
  ...
  // tz.TZDateTime _nextQuarterlyDate(
  //     tz.TZDateTime scheduledDate, int day, TimeOfDay time) {
  ...
  // void _scheduleQuarterlyNotification(int id, String title, String body,
  //     tz.TZDateTime scheduledDate, TimeOfDay time) {
  //   Timer(
  //     scheduledDate.difference(tz.TZDateTime.now(tz.local)),
  //     () async {
  //       try {
  //         tz.TZDateTime nextQuarterlyDate =
  //             _nextQuarterlyDate(scheduledDate, scheduledDate.day, time);

  //         print(
  //             'Scheduling next quarterly notification at: $nextQuarterlyDate');
  //         await scheduleNotification(
  //             id, title, body, nextQuarterlyDate, time, Repeat.Quarterly);
  //       } catch (e) {
